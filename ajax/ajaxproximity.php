...
if(isset($_REQUEST['no_parada'])){
	$no_parada = (int)$_REQUEST['no_parada'];
	$resultados = 11;
}
else{
	$no_parada = 0;
...
$paradas = obtener_paradas_proximas($lat, $long, $radio, $resultados);
$texto = '';
$res_paradas = array();
$output = array();
$contador = 0;
if($paradas != false && count($paradas) > 0){ //Hay resultados que mostrar
     foreach($paradas as $parada){
         if((int)$no_parada != (int)$parada['nombre']){
    ...
    $output['html'] = '<p class="info_general">No se ha encontrado ninguna parada a menos de '.$radio.' m de su posición</p>';
    $fecha = date('j/n/Y H:i:s');
    $output['fecha'] = 'Última búsqueda: '.$fecha;
    echo json_encode($output);
}
