...
function infoParada($idparada){
	...
	try{
		...
		$client = new nusoap_client('http://isae.aucorsa.es/services/dinamica.asmx?WSDL',true);
		$data = array(
			"status"=>0,
			"parada"=>$idparada,
			"linea"=>"*"
		...
		if($info['GetPasoParadaResult']['PasoParada'][0]){
			foreach($info['GetPasoParadaResult']['PasoParada'] as $lineabus){
				...
				$lineas[]=$linea_nomre;
				$recorrido[]=utf8_encode($lineabus['ruta']);
			...
			$lineas[]=$linea_nomre;
			$recorrido[]=utf8_encode($info['GetPasoParadaResult']['PasoParada']['ruta']);
