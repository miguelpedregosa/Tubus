...
function infoParada($idparada){
	...
	try{
		...
		$client = new nusoap_client('http://www.ayto-santander.es:9001/services/dinamica.asmx?WSDL',true);
		$data = array(
			"status"=>0,
			"parada"=>$idparada,
			"linea"=>"*"
		...
		if(!isset($info['GetPasoParadaResult']) OR !isset($info['GetPasoParadaResult']['PasoParada'])) return -1;
		
		if(!isset($info['GetPasoParadaResult']['PasoParada'][0])){ 
			$aux = $info['GetPasoParadaResult']['PasoParada'];
			$info['GetPasoParadaResult']['PasoParada'] = '';
			$info['GetPasoParadaResult']['PasoParada'][] = $aux;				 
		}
		...
		if(isset($info['GetPasoParadaResult']['PasoParada'][0]) && !empty($info['GetPasoParadaResult']['PasoParada'][0])){
			foreach($info['GetPasoParadaResult']['PasoParada'] as $lineabus){
				...
				$lineas[]=$linea_nomre;
				$recorrido[]=(string)utf8_encode($lineabus['ruta']);
